...
const DownloadButton = ({ snippet }: { snippet?: Snippet }) => {
  if (!snippet || !snippet.content || !snippet.name || !snippet.extension) return null;
...
export const SnippetDetail = (props: SnippetDetailProps) => {
  ...
  return (
    <Box p={4} minWidth={'60vw'}>
        ...
        isLoading ? (<>
          ...
          <Box display="flex" flexDirection="row" gap="8px" padding="8px">
            ...
            <Tooltip title={"Save changes"}>
              <IconButton color={"primary"} onClick={() => updateSnippet({ id: id, updateSnippet: { content: code } })} disabled={isUpdateSnippetLoading || (snippet && !Array.isArray(snippet) && snippet.content === code)}>
                <Save />
              </IconButton>
            </Tooltip>
      ...
      <DeleteConfirmationModal open={deleteConfirmationModalOpen} onClose={() => setDeleteConfirmationModalOpen(false)} id={snippet && !Array.isArray(snippet) ? snippet.id : ""} setCloseDetails={handleCloseModal} />
    </Box>
  );
}